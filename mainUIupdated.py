...
class Ui_MainWindow(object):
    ...
    def save_data(self, jsData):
        with open(self.file_path, 'r+') as jsFileObj:
            ...
            last_entry_from_input = list(jsData[self.username].keys())[-1]
            print(jsData[self.username][last_entry_from_input])
            
            last_entry_from_data = list(data[self.username].keys())[-1]
    ...
    def _checkIfBasicInput(self):
        
        result = self.DataHandler.checkIfBasicInput()
        ...
        if result[0]:
            ...
            self.totalMoneyInput.setText(str(result[1][self.username]['basics']['total']))
        
            if result[1][self.username]['basics']['monthly_savings'] != 0:
                    self.savingsTypeCombo.setCurrentIndex(1)
                
            elif result[1][self.username]['basics']['daily_savings'] != 0:
    ...
    def get_basic_input(self):
        ...
        try:
            saving_type = self.savingsTypeCombo.currentText().strip()
            monthly_savings = 0
            daily_savings = 0
            random = True
            total = int(self.totalMoneyInput.text().strip())          
            ...
            if saving_type == 'Random':
                ...
                monthly_savings = int(self.savings.text().strip())
                daily_savings = 0
                random = False
           
            elif saving_type == 'Daily':
                daily_savings = int(self.savings.text().strip())
                monthly_savings = 0
                random = False
            basic_data = {self.username:{"basics": {'date-registered':date.today(),'total': total, 'monthly_savings': monthly_savings, 'daily_savings': daily_savings}}}
    ...
    def get_daily_input(self):
        ...
        add_savings = 0
        withd_savings = 0
        total_left = int(self.total_left_input.text().strip())
        add_savings = int(self.add_savings.text().strip())
        withd_savings = int(self.withd_savings.text().strip())
        description = str(self.DescriptionEdit.toPlainText().strip())
        with open(self.file_path, 'r') as jsFile:
            data = json.load(jsFile)
            key = list(data[self.username].keys())[-1]
            if len(list(data[self.username].keys())) >= 2:
                key_no = key[6:]
            else:
                key_no = 0
            total = data[self.username]['basics']['total']
            monthly_savings = data[self.username]['basics']['monthly_savings']
            daily_savings = data[self.username]['basics']['daily_savings']
        today = date.today()
        daily_data = {'key_no': key_no ,'today': today, 'total': total, 'total_left': total_left, 'add_savings': add_savings, 'withd_savings': withd_savings, 'description': description}
        return daily_data
    ...
    def evaluate_data(self, data):
        with open(self.file_path, 'r') as jsFile:
            ...
            key = list(jsData[self.username].keys())[-1]
            
            if key == 'basics':
                total = data['total']
                date = str(data['today'])
                total_left_to_spend = data['total_left']
                ...
                date = str(data['today'])
                total_spent = jsData[self.username][key]['total_spent']
                total_left_to_spend = jsData[self.username][key]['total_left_to_spend']
                savings_total = jsData[self.username][key]['savings_total']
                
                if data['total_left'] > total_left_to_spend:
                    gained = data['total_left'] - total_left_to_spend
                    total_left_to_spend += gained
                
                elif total_left_to_spend > data['total_left']:
                    spent = jsData[self.username][key]['total_left_to_spend'] - data['total_left']
                    total_spent += spent
                ...
                if data['add_savings'] != 0:
                    ...
                    total_left_to_spend = total_left_to_spend - spent
                
                savings_total = savings_total + (data['add_savings'] - data['withd_savings'])
                total = data['total']
                jsData[self.username][f'entry_{current_entry_no}'] = {
                                                    "date": date,
                                                    "spent": spent,
                                                    "gained": gained,
                                                    "saved_today": saved_today,
    ...
    def save_daily_input(self):
        ...
        confirmationMsg = QMessageBox()
        confirmationMsg.setWindowTitle("Confirmation")
        confirmationMsg.setText(f"The data was saved successfully as entry number {str(int(data['key_no']) +1)}")
        _ = confirmationMsg.exec()
        
        with open(self.file_path, 'r') as jsObj:
            jsData = json.loads(jsObj.read())
            keys = list(jsData[self.username].keys())
            if jsData != {}:
                last_key = keys[-1]
                if last_key == 'basics':
                    ...
                    self.dateSelect.setItemText(0, "Most recent")
                    for key in keys[1:]:
                        self.dateSelect.addItem("")
                        date = jsData[self.username][key]['date']
                        self.dateSelect.setItemText(keys.index(key), date)
    ...
    def removeData(self):
        ...
        warningMsg.setStandardButtons(QMessageBox.StandardButton.Cancel | QMessageBox.Ok)
    ...
    def display_data(self):
        ...
        with open(self.file_path, 'r') as jsObj:
            ...
            keys = list(jsData[self.username].keys())[1:]
            if dateSelected != "Most recent":
                for key in keys:
                    if jsData[self.username][key]['date'] == dateSelected:
                        data = jsData[self.username][key]
    ...
    def setupUi(self, MainWindow):
        ...
        MainWindow.setStyleSheet("background-color: rgb(0, 18, 25);")
        ...
        self.tabWidget1.setStyleSheet("background-color: rgb(10, 147, 150);\n"
"border-color: rgb(0, 18, 25);\n"
"")
        ...
        self.add_savings.setGeometry(QtCore.QRect(220, 110, 171, 41))
        ...
        self.withd_savings.setGeometry(QtCore.QRect(220, 190, 171, 41))
        ...
        self.DescriptionEdit = QtWidgets.QTextEdit(self.tab_3)
        self.DescriptionEdit.setGeometry(QtCore.QRect(220, 250, 171, 71))
        self.DescriptionEdit.setStyleSheet("background-color:rgb(167, 254, 255);")
        self.DescriptionEdit.setObjectName("DescriptionEdit")
        self.label_6 = QtWidgets.QLabel(self.tab_3)
        self.label_6.setGeometry(QtCore.QRect(20, 270, 131, 20))
        font = QtGui.QFont()
        font.setPointSize(14)
        ...
        self.headerLabel.setStyleSheet("background-color: rgb(10, 147, 150);")
        ...
        self.dateSelect.setStyleSheet("background-color: rgb(184, 246, 255);")
        ...
        with open(self.file_path, 'r') as jsFile:
            ...
            if jsData != {}:
                last_key = list(jsData[self.username].keys())[-1]
                if last_key == 'basics':
                    self.dateSelect.setEnabled(False)
                elif 'entry_' in last_key:
                    self.dateSelect.addItem("")
        ...
        self.underheaderLine_2.setStyleSheet("background-color: rgb(10, 147, 150);\n"
"border-color: rgb(0, 18, 25);\n"
"")
        ...
        self.headerLabel_2.setStyleSheet("background-color: rgb(10, 147, 150);\n"
"border-color: rgb(0, 18, 25);\n"
"")
